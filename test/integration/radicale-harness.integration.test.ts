...
describe('Radicale authenticated discovery', () => {
	issue53It(
		['CONNECTION-001', 'DISCOVERY-001'],
		'discovers the generated current-user principal and calendar home from fresh storage',
		async () => {
	...
	issue53It(
		['AUTH-401-001'],
		'rejects an invalid password without invalidating the generated credential',
		async () => {
			const run = await startRun();
			try {
				await expect(
	...
	issue53It(
		['HARNESS-006'],
		'reports bounded sanitized unavailability and still permits teardown',
		async () => {
			const run = await startRun();
	...
	issue53It(
		['DISCOVERY-002', 'CALENDAR-GET-001'],
		'discovers real writable and read-only VEVENT calendars with accurate privileges',
		async () => {
			const run = await startRun();
			try {
	...
	issue53It(
		['CALENDAR-MANY-001'],
		'executes Calendar Get Many for an empty home and filters/sorts writable, read-only, and non-event collections',
		async () => {
			const run = await startRun();
			try {
				await expect(harness.waitForAuthenticatedReadiness(run)).resolves.toBeUndefined();
	...
	issue53It(
		['CALENDAR-MANY-002'],
		'loads duplicate, missing, empty, writable, and read-only VEVENT calendars while excluding VTODO-only collections',
		async () => {
			const run = await startRun();
			try {
	...
	issue53It(
		['EVENT-MANY-RANGE-001', 'EVENT-MANY-RECURRENCE-001'],
		'enforces [start,end) overlap boundaries and returns one recurring resource',
		async () => {
			const run = await startRun();
			try {
	...
	issue53It(
		['EVENT-CREATE-IANA-001', 'EVENT-UPDATE-TIME-001'],
		'creates, queries, updates bounds, changes zones, and reads generated finite fallback definitions',
		async () => {
			const run = await startRun();
			try {
...
describe('Radicale recurring event authoring and preservation', () => {
	issue53It(
		...
		'creates, queries, updates, and reads one recurring master without expansion',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
...
describe('Radicale calendar-event UID resolution', () => {
	issue53It(
		...
		'retrieves one stored event identically by exact resource URL and UID with live missing and forbidden boundaries',
		async () => {
			const run = await startRun();
			try {
...
describe('Radicale collision-safe Event Create', () => {
	issue53It(
		...
		'round-trips multiple alarms through targeted edit, remove, and add',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
	...
	issue53It(
		...
		'generates a canonical UUID for blank node UID and reads back one matching VEVENT identity',
		async () => {
			const run = await startRun();
			try {
	...
	issue53It(
		['EVENT-CREATE-STRUCTURED-UTC-001', 'CREATE-COLLISION-001'],
		'creates and reads back Unicode data, then preserves it across a same-UID collision',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
	...
	issue53It(
		['READONLY-CREATE-001'],
		'maps read-only denial and rejects invalid input before any live request',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = await createSyntheticCalendar(
...
describe('Radicale conditional Event Update operation', () => {
	...
	issue53It(
		['EVENT-UPDATE-URL-001', 'EVENT-UPDATE-PRESERVE-001'],
		'updates a direct Resource URL with the exact fetched and authoritative ETags while preserving unknown data',
		async () => {
			const run = await startRun();
			try {
	...
	issue53It(
		...
		'updates by UID with an exact caller ETag through REPORT -> PUT -> GET',
		async () => {
			const run = await startRun();
			try {
	...
	issue53It(
		...
		'maps a stale caller ETag to one terminal conflict with no read-back',
		async () => {
			const run = await startRun();
			try {
				...
				expect(await retained.text()).toContain(
					'SUMMARY:Newer resource retained after stale Update',
				);
			} finally {
				await teardownRun(run);
			}
	...
	issue53It(
		['READONLY-UPDATE-001'],
		'maps read-only denial, performs no read-back, and retains the event',
		async () => {
			const run = await startRun();
			try {
...
describe('Radicale Event Delete node operation', () => {
	issue53It(
		...
		'deletes an exact Resource URL with the resolved ETag and returns stable metadata',
		async () => {
			const run = await startRun();
			try {
	...
	issue53It(
		...
		'resolves and deletes by UID within only the selected calendar',
		async () => {
			const run = await startRun();
			try {
	...
	issue53It(
		...
		'maps a supplied stale ETag to one terminal conflict and retains the newer resource',
		async () => {
			const run = await startRun();
			try {
				...
				expect(retained.status).toBe(200);
				expect(await retained.text()).toContain(
					'SUMMARY:Newer resource retained after stale delete',
				);
			} finally {
				await teardownRun(run);
			}
	...
	issue53It(
		...
		'reports a missing event without issuing DELETE',
		async () => {
			const run = await startRun();
			try {
	...
	issue53It(
		['READONLY-DELETE-001'],
		'maps read-only denial and retains the resource while every DELETE stays conditional',
		async () => {
			const run = await startRun();
			try {
...
describe('Radicale issue #41 all-day event interoperability', () => {
	issue53It(
		['EVENT-CREATE-ALLDAY-001', 'EVENT-MANY-ALLDAY-001'],
		'round-trips one-day leap and multi-day year-boundary values and enforces half-open queries',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
...
describe('Radicale extended VEVENT metadata round trip', () => {
	issue53It(
		...
		'preserves, replaces, removes, and upserts metadata with authoritative URL and ETag values',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
...
describe('Radicale deterministic Event Upsert', () => {
	issue53It(
		...
		'creates then updates one supplied UID with preservation-first conditional requests and no DELETE',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
	...
	issue53It(
		...
		'creates then updates an IANA event with one finite fallback definition and no DELETE',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
	...
	issue53It(
		...
		'creates two omitted-UID resources through conditional PUTs for timed UTC and all-day input',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
	...
	issue53It(
		...
		'maps a REPORT-to-PUT stale race to one conflict and leaves the winner intact',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
				...
				const request = vi.fn(async (input: CalDavTransportRequest) => {
					if (input.method === 'PUT' && !raced) {
						raced = true;
						expect((await authenticatedFetch(run, resourceUrl, 'PUT', winnerBody)).status).toBe(
							204,
						);
					}
	...
	issue53It(
		['READONLY-UPSERT-001'],
		'uses one conditional Create attempt on a read-only calendar and never DELETEs',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
...
describe('Radicale validated Raw ICS writes', () => {
	issue53It(
		['EVENT-CREATE-RAW-001', 'EVENT-UPDATE-RAW-001', 'EVENT-UPSERT-RAW-001'],
		'creates, fully replaces and upserts complex event objects with authoritative read-back and stale-ETag safety',
		async () => {
			const run = await startRun();
			try {
				const calendarUrl = validateAbsoluteHttpUrl(
				...
				const methods = request.mock.calls.map(
					([input]) => (input as CalDavTransportRequest).method,
				);
...
describe('Radicale run isolation and confinement', () => {
	issue53It(
		['HARNESS-001'],
		'resets only the selected run and preserves authenticated reconnection',
		async () => {
	...
	issue53It(
		['HARNESS-002'],
		'allocates distinct parallel identities and confines each runtime',
		async () => {
...
describe('Radicale mandatory failure cleanup', () => {
	issue53It(
		['HARNESS-003', 'QUALITY-003'],
		'returns nonzero only after deliberate failure cleanup and redacts the run secret',
		async () => {
